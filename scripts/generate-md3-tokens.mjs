...
function parseArgs(argv) {
    ...
    if (!/^#[0-9a-f]{6}$/.test(args.source)) fail(`--source must be lowercase '#rrggbb', got '${args.source}'`);
...
async function main() {
    ...
    for (const mode of ['light', 'dark']) {
        const mapped = Object.keys(resolveMapping(mode, contrast.group));
    ...
    const slug = `md3-${VARIANT_SLUGS[args.variant]}-${args.source}`;
    ...
    for (const file of FILES) {
        ...
        const lightMapping = resolveMapping('light', contrast.group);
        const darkMapping = resolveMapping('dark', contrast.group);
